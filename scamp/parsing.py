...
spelling_policy = "flats" / "sharps" / (white_key r'[-_]*' accidental? r'[-_]*' mode?) &("," / EOF)
...
property = articulations / notations / noteheads / playback_adjustments / spelling_policy / 
           dynamics / extra_playback_parameter / texts
...
""".format(
    ...
    all_dynamics=_join_with_quotes_and_slashes(pymusicxml.Dynamic.STANDARD_TYPES)
)
...
class PropertiesVisitor(PTNodeVisitor):
    ...
    def visit_articulations(self, node, children): return {"articulations": children}

    def visit_notations(self, node, children): return {"notations": children}

    def visit_noteheads(self, node, children): return {"noteheads": children}

    def visit_dynamics(self, node, children): return {"dynamics": children}
    ...
    def visit_playback_adjustments(self, node, children): return {"playback_adjustments": children}
    ...
    def visit_spelling_policy(self, node, children):
        ...
        children = [term for term in children if "-" not in term and "_" not in term]
        return {"spelling_policy": SpellingPolicy.from_string("".join(children))}
