...
class ParameterChangeSegment(EnvelopeSegment):
    ...
    def __init__(self, parameter_change_function, start_value, target_value, transition_length, transition_curve_shape,
                 time_increment=0.01, callback_function=None):
    ...
    def run(self, clock):
        ...
        def _synchronized_function(child_clock):
            ...
            # only run the callback function if it completes without being aborted. This is used to chain together
            # multiple stages of parameter change, and if it's interrupted at any stage, we want to end the chain.
    ...
    def change_note_parameter(self, note_id, param_name, target_value, transition_length=0,
        ...
        if hasattr(target_value, "__len__"):
            ...
            for i in range(len(target_value)):
                parameter_change_segments.append(
                    ParameterChangeSegment(parameter_change_function,
                                           note_info["parameter_values"][param_name] if i == 0 else target_value[i - 1],
                                           target_value[i], transition_length[i], transition_curve_shape[i])
                )

            for pc_segment, next_pc_segment in zip(parameter_change_segments[:-1], parameter_change_segments[1:]):

                def callback():
                    segments_list.append(next_pc_segment)
                    next_pc_segment.run(clock)

                pc_segment.callback_function = callback

            segments_list.append(parameter_change_segments[0])
            parameter_change_segments[0].run(clock)
        else:
            pcs = ParameterChangeSegment(parameter_change_function, note_info["parameter_values"][param_name],
                                         target_value, transition_length, transition_curve_shape)
            segments_list.append(pcs)
            pcs.run(clock)

    def change_note_pitch(self, note_id, pitch_target, transition_length=0, transition_curve_shape=None):
        """
        Changes the pitch of the given note; can take a single target / transition length or a list of multiple
        :param note_id: note_id of the note whose pitch to change
        :param pitch_target:
        ...
        if transition_length == 0:
            ...
            self._note_info_by_id[note_id]["pitch_change_segments"].append((
                time_stamp, time_stamp, pitch_target
            ))
            return

        if hasattr(pitch_target, "__len__"):
            # assume linear segments unless otherwise specified
            transition_curve_shape = [0] * len(pitch_target) \
                if transition_curve_shape is None else transition_curve_shape
            assert hasattr(transition_length, "__len__") and hasattr(transition_curve_shape, "__len__") and \
                   len(pitch_target) == len(transition_length) == len(transition_curve_shape), \
                "A list of pitch targets must be accompanied by a equal length list of transition lengths and shapes."
            _pitch_change_processes = [
                self._get_param_change_process(note_id, "pitch", target, length, shape, clock)
                for target, length, shape in zip(pitch_target, transition_length, transition_curve_shape)
            ]

            def _pitch_change_process(child_clock):
                for x in _pitch_change_processes:
                    x(child_clock)

        else:
            transition_curve_shape = 0 if transition_curve_shape is None else transition_curve_shape
            _pitch_change_process = self._get_param_change_process(note_id, "pitch", pitch_target, transition_length,
    ...
    def _get_param_change_process(self, note_id, param_name, target, transition_length, transition_curve_shape, clock):
        ...
        if transition_length == 0:
            ...
            def _param_change_process(child_clock):
                ...
                curve = EnvelopeSegment(0, transition_length, note_info["parameter_values"][param_name], target,
                ...
                def animate_param():
                    nonlocal segment_aborted, beats_passed

                    while beats_passed < transition_length:
                        if this_animation_id != note_info["current_animation_ids"][param_name]:
                            ...
                            break
                        do_change_parameter(note_id, curve.value_at(beats_passed))
                        time.sleep(time_increment)
                        beats_passed += time_increment * clock.absolute_rate()

                    if segment_aborted:
                        # if we're aborting early, add a (truncated) segment to the segments_list
                        child_clock.kill()
                        segments_list.append((
                            start_time_stamp, TimeStamp(child_clock), curve.value_at(beats_passed)
                        ))
                ...
                print("STARTING SEGMENT ", this_animation_id)
                # child_clock.fork_unsynchronized(animate_param)
                child_clock.wait(transition_length)
                print("SEGMENT", this_animation_id, "FINISHED")
